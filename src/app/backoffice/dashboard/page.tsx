  ...
  return (
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        ...
        <Tabs defaultValue="guide" onValueChange={(v) => setRole(v as "traveler" | "guide")} className="w-full md:w-auto">
          <TabsList className={cn(
            "border p-1.5 rounded-2xl h-auto transition-colors",
            isDark ? "bg-zinc-900 border-zinc-800" : "bg-white border-zinc-200"
          )}>
            <TabsTrigger value="traveler" className={cn(
              "font-black px-8 py-3 rounded-xl transition-all text-sm uppercase tracking-widest",
              isDark 
      ...
      <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard title={t('metrics.escrow')} value="$24,850" sub="Across 12 trips" icon={<Wallet className="h-6 w-6" />} color="blue" isDark={isDark} />
        <MetricCard title={t('metrics.liveSessions')} value="42" sub="Chatting right now" icon={<Clock className="h-6 w-6" />} color="purple" isDark={isDark} />
        <MetricCard title={t('metrics.matchRate')} value="94%" sub="+5.2% from last week" icon={<CheckCircle2 className="h-6 w-6" />} color="emerald" isDark={isDark} />
        <MetricCard title={t('metrics.feedback')} value="4.92" sub="Verified reviews only" icon={<Briefcase className="h-6 w-6" />} color="amber" isDark={isDark} />
      </div>

      <div className="grid gap-10 lg:grid-cols-3">
        <Card className={cn(
          "lg:col-span-2 rounded-[3rem] overflow-hidden shadow-3xl border transition-colors",
          isDark ? "bg-zinc-900 border-zinc-800" : "bg-white border-zinc-200"
        )}>
          <CardHeader className={cn("p-10 border-b transition-colors", isDark ? "border-zinc-800" : "border-zinc-100")}>
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <CardTitle className={cn("text-3xl font-black uppercase tracking-tight font-rounded", isDark ? "text-white" : "text-zinc-900")}>{t('recentTransactions')}</CardTitle>
              ...
              {[
                ...
                <div key={tx.id} className="p-8 flex items-center justify-between hover:bg-primary/[0.02] transition-colors group">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <p className={cn("font-black text-xl font-rounded", isDark ? "text-white" : "text-zinc-900")}>{tx.name}</p>
                      <p className="text-zinc-500 text-xs font-bold uppercase tracking-widest mt-1">Guide: {tx.guide} • ID: {tx.id}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className={cn("font-black text-2xl font-rounded", isDark ? "text-white" : "text-zinc-900")}>{tx.amount}</p>
                    <Badge className={cn(
          ...
          <CardFooter className={cn("p-8 flex justify-center border-t transition-colors", isDark ? "bg-zinc-950/50 border-zinc-800" : "bg-zinc-50 border-zinc-100")}>
             <Link href="#" className="flex items-center gap-3 text-zinc-400 hover:text-primary transition-colors font-black text-xs uppercase tracking-[0.3em]">
        ...
        <Card className={cn(
          "rounded-[3rem] overflow-hidden shadow-3xl flex flex-col border transition-colors",
          isDark ? "bg-zinc-900 border-zinc-800" : "bg-white border-zinc-200"
        )}>
          <CardHeader className={cn("p-10 border-b transition-colors", isDark ? "bg-zinc-950/50 border-zinc-800" : "bg-zinc-50 border-zinc-100")}>
            <CardTitle className={cn("text-2xl font-black uppercase tracking-tight font-rounded", isDark ? "text-white" : "text-zinc-900")}>{t('marketIntelligence')}</CardTitle>
             ...
             {[
               ...
               <div key={city.city} className="flex items-center justify-between group">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <p className={cn("font-black text-xl font-rounded", isDark ? "text-white" : "text-zinc-900")}>{city.city}</p>
          ...
          <div className="p-10 pt-0">
             <Button className={cn(
               "w-full h-16 rounded-2xl font-black uppercase tracking-[0.2em] text-sm shadow-xl transition-all",
...
function MetricCard({ title, value, sub, icon, color, isDark }: { title: string, value: string, sub: string, icon: React.ReactNode, color: string, isDark: boolean }) {
  ...
  return (
    <Card className={cn(
      "p-8 rounded-[2.5rem] transition-all border-l-4 shadow-2xl group border",
      isDark ? "bg-zinc-900 border-zinc-800" : "bg-white border-zinc-200"
      ...
      <div className="space-y-2">
        <h3 className={cn("text-4xl font-black font-rounded", isDark ? "text-white" : "text-zinc-900")}>{value}</h3>
        <p className="text-[11px] font-black text-zinc-500 uppercase tracking-widest">{sub}</p>
      </div>
    </Card>
  )
}
